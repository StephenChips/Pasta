...
1. integer
    1. real(integer: num): real;
    2. istr(integer: num): string;
...
A field can not only be a primitive type,  but also can be a compound type, for example, we can define a nested record, whose field is another record.
...
2. A undetermined array must be initialized before using (e.g. indexing, assigns). The approachs to initialize a undetermined array is assign a determined array to it. Otherwise, the compiler should complain and raise a error at compiling time.
...
**Program** **Function** **Begin** **End** **For** **While**
**Repeat** **Integer** **Until** **If** **Else** **Inc** **Dec** 
**Ref**  **Real** **String** **Boolean** **Character** **Array**
**Enumerate**  **Record** **Write** **Read**
#### Arthmetic Operators
